...
import { coachPhoto } from "@/components/mcg-3/academy-photo";
...
export const SlotButton = ({
    ...
    // MCG's note: booked times should still be visible, but they blended into the open
    // ones. Taken slots go flat and grey; open slots get a light brand tint and a border
    // so a glance finds them.
    if (status !== "open") {
        return (
            <div className="flex flex-col items-center justify-center gap-0.5 rounded-lg bg-tertiary px-2 py-2.5 text-center opacity-70">
    ...
    return (
        <button
            ...
            className={cx(
                ...
                "focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus-ring",
                selected ? "bg-brand-solid text-white ring-transparent" : "bg-brand-primary shadow-xs ring-brand hover:bg-primary_hover hover:ring-2",
            )}
        >
 ...
 * underneath.
 */
export const GroupSizePriceTable = ({ service, coach, size = "sm", narrow }: { service: LessonService; coach?: Coach; size?: "sm" | "md"; /** Two columns, for a sidebar. */ narrow?: boolean }) => {
    ...
    return (
        <div className="flex flex-col gap-1.5">
            ...
            <div
                className={cx(
                    "grid gap-1.5",
                    narrow ? "grid-cols-2" : [rows.length === 2 && "grid-cols-2", rows.length === 3 && "grid-cols-3", rows.length >= 4 && "grid-cols-2 sm:grid-cols-4"],
                )}
            >
                {rows.map((row) => (
