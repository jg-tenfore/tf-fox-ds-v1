...
/**
 * `/instruction/service/[id]` (Prototype 2) — the service-first path, Square-style.
 *
...
import { CoachAvatar, GroupSizePriceTable, StepRail } from "@/components/instruction-2/instruction-ui";
...
export const ServiceScreen = ({ serviceId, initialCourse = ALL_COURSES }: { serviceId: string; initialCourse?: string }) => {
    ...
    const continueHref = choice
        ? bookHref({ coachId: choice, serviceId: service.id, courseSlug: allCourses ? (chosenCoach?.courseSlugs[0] ?? undefined) : course })
        : undefined;
