...
import { CapacityMeter, CourseChip, CreditPips, MenuItemRow, MetaLine, MicroLabel, SectionTitle, SlotButton } from "@/components/instruction-3/instruction-ui";
import { type AcademyInstructor, academyBanner, academyDisplayName, academyInstructor, academyLogo, instructorPhoto } from "@/components/mcg-3/academy-roster";
...
import { PackChooser } from "../instruction/credits-explainer";
import {
            ...
            <McgShell>
                <McgPage width="3xl">
                    <div className="flex flex-col items-start gap-4 rounded-2xl bg-primary p-8 ring-1 ring-secondary ring-inset">
                        ...
                        <Button size="md" color="secondary" href="/instruction/?view=instructors" iconLeading={ArrowLeft}>
                            All instructors
                        </Button>
                    </div>
                </McgPage>
    ...
    if (instructor.comingSoon) {
        return (
            <McgShell>
                <McgPage width="6xl">
                    <Link
                        href="/instruction/?view=instructors"
                    ...
                    <div className="grid grid-cols-1 gap-10 lg:grid-cols-2">
                        ...
                        <div className="flex flex-col items-start gap-4">
                            ...
                            <div className="flex flex-wrap gap-3">
                                ...
                                <Button size="lg" color="secondary" href="/instruction/?view=instructors" iconLeading={ArrowLeft}>
                                    All instructors
                                </Button>
                            </div>
                        </div>
        ...
        <McgShell>
            <McgPage width="7xl">
                ...
                <div className="mt-12 flex flex-col gap-6">
                    {/* ---- Everything this instructor offers, one tap from the top ---- */}
                    <nav aria-label={`What ${instructor.name} offers`} className="flex flex-wrap items-center gap-2 rounded-2xl bg-primary p-4 ring-1 ring-secondary ring-inset">
                        <span className="mr-1 text-sm font-semibold text-primary">What {instructor.name.split(" ")[0]} offers</span>
                        {[
                            { href: "#reservations", label: "Private lessons", count: privates.length },
                            { href: "#programs", label: "Group clinics", count: programs.length },
                            { href: "#packages", label: "Packages", count: packages.length + (subscription ? 1 : 0) },
                        ]
                            ...
                            .map((item) => (
                                <a
                                    key={item.href}
                                    href={item.href}
                                    className="flex items-center gap-1.5 rounded-full bg-secondary px-3 py-1.5 text-sm font-semibold text-secondary ring-1 ring-secondary transition duration-100 ease-linear ring-inset hover:ring-brand"
                                >
                                    {item.label}
                                    <span className="text-tertiary tabular-nums">{item.count}</span>
                                </a>
                            ))}
                    </nav>
                    ...
                    {/* ---- Packages: buy ahead, spend later. Never part of a booking. ---- */}
                    {(packages.length > 0 || subscription) && (
                        <Panel id="packages">
                            <SectionTitle sub={`Prepay for lessons with ${instructor.name} and spend the credits whenever you book.`}>Packages</SectionTitle>

                            <PackChooser coach={coach} heading={`${instructor.name.split(" ")[0]}'s lesson packs`} />
                            ...
                            {subscription && (
                                <div className="flex flex-col gap-3 rounded-xl bg-brand-primary p-5 ring-1 ring-brand ring-inset">
                                    ...
                                    <span className="text-xs text-tertiary tabular-nums">
                                        {money0(subscription.listMonthly)} booked one at a time · save {money0(subscription.listMonthly - subscription.monthlyPrice)} a month
                                    </span>
                                    ...
                                    <Button size="sm" color="primary" href="/instruction/packages" className="mt-auto w-fit">
                                        Start monthly plan
                                    </Button>
                                </div>
                            )}
                                ...
                                {programs.map((program) => {
                                    ...
                                    return (
                                        <div key={program.id} className="flex flex-col gap-3 rounded-xl bg-secondary p-5 ring-1 ring-secondary ring-inset">
                                            ...
                                            <Button
                                                size="sm"
                                                color={full ? "secondary" : "primary"}
                                                href={bookHref({ coachId: instructor.id, serviceId: program.id, courseSlug: program.courseSlug })}
