...
import { LessonBookingFlow } from "@/components/instruction-3/lesson-booking-flow";
import { InstructionShellProvider } from "@/components/instruction-3/instruction-ui";
import { coachById, serviceById, PRIVATE_SERVICES } from "@/components/instruction-3/instruction-catalog";
import { McgHero, McgPage, McgShell } from "@/components/mcg-3/mcg-chrome";
import { SquareBookingFlow } from "@/components/mcg-3/instruction/square-booking";
import type { ReactNode } from "react";

const KEY = "mcg-p3-lesson-selection-v1";

export interface LessonSelection {
    coachId: string;
    serviceId: string;
    courseSlug?: string;
    /** The dates asked for on the search step, carried through so nothing is re-asked. */
    from?: string;
    to?: string;
}
...
export const bookHref = ({ coachId, serviceId, courseSlug, from, to }: LessonSelection) => {
...
const useSelection = (): { selection: LessonSelection | null; ready: boolean } => {
    ...
    useEffect(() => {
        ...
        if (coachId && coachById(coachId) && serviceId && serviceById(serviceId)) {
            setSelection({ coachId, serviceId, courseSlug, from: params.get("from") ?? undefined, to: params.get("to") ?? undefined });
            setReady(true);
            return;
        }
        ...
        <McgPage width="5xl">
            <div className="flex flex-col items-center gap-4 rounded-2xl bg-primary px-6 py-14 text-center ring-1 ring-secondary ring-inset">
                ...
                <div className="flex flex-wrap justify-center gap-2.5">
                    ...
                    <Button size="lg" color="secondary" href="/instruction/?formats=clinic,junior-camp,junior-league,op36">
...
export const LessonBookScreen = ({ selection: given }: LessonBookScreenProps = {}) => {
    ...
    const service = serviceById(selection.serviceId)!;
    const courseSlug = selection.courseSlug ?? service.courseSlug ?? coach.courseSlugs[0];

    // Private lessons book the Square way; a group program keeps the existing flow,
    // since its dates are fixed and there is no time to choose.
    if (service.kind === "private") {
        return <SquareBookingFlow service={service} coachId={selection.coachId} courseSlug={courseSlug} searchFrom={selection.from} searchTo={selection.to} />;
    }

    return (
        <InstructionShellProvider shell={routedShell}>
            <LessonBookingFlow locked step="details" coachId={selection.coachId} serviceId={selection.serviceId} courseSlug={courseSlug} />
