...
import {
    ...
    sessionRegistered,
    spotsLeft,
} from "@/components/mcg-3/events-catalog";
import { BookingQuestions, EligibilityFields, MultiBuyBanner, RequirementsPanel } from "@/components/mcg-3/registration/registration-ui";
...
import {
    ...
    multiBuyLabel,
    quoteMultiBuy,
} from "@/components/mcg-3/registration-rules";
...
const RegistrationSheet = ({
    ...
    const { addToCart, user } = useSession();
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
            ...
            <div role="dialog" aria-label={`Register for ${clinic.title}`} className="relative flex h-full w-full max-w-xl flex-col bg-primary shadow-xl duration-300 animate-in slide-in-from-right">
                ...
                <div className="flex flex-1 flex-col gap-6 overflow-y-auto p-6">
                    {/* Registrations are attached to an account too — same rule as a lesson. */}
                    <SignInGate what="registration" email={user?.email ?? email} onEmail={setEmail} />
                ...
                <div className="flex flex-col gap-3 border-t border-secondary p-6">
                    ...
                    <Button size="lg" color="primary" isDisabled={!user || blocked || incomplete} onClick={submit}>
                        Add to cart
                    </Button>
                    <p className="text-center text-xs text-tertiary">
                        {!user
                            ? "Sign in above to register — everything you've entered is kept."
                            : blocked
                              ? "Someone above can't be registered for this clinic."
                              : incomplete
    ...
    if (waitlisted) {
        return (
            <div className="sticky top-6 rounded-2xl bg-primary p-5 shadow-lg ring-1 ring-secondary">
                ...
                <Button size="lg" color="secondary" href="/instruction/?formats=clinic,junior-camp,junior-league,op36" className="mt-4 w-full">
                    Back to all clinics
                </Button>
            </div>
        );
...
export const ClinicDetailScreen = ({ clinicId, initialSessions = [], initialRegistration }: ClinicDetailScreenProps) => {
        ...
        return (
            <McgShell>
                <McgPage width="3xl">
                    ...
                    <Button size="lg" href="/instruction/?formats=clinic,junior-camp,junior-league,op36" className="mt-6" iconLeading={ArrowLeft}>
                        All instruction
                    </Button>
                </McgPage>
            </McgShell>
    ...
    return (
        <McgShell>
            <McgPage width="6xl">
                <Link href="/instruction/?formats=clinic,junior-camp,junior-league,op36" className="mb-5 flex w-fit items-center gap-1.5 text-sm font-semibold text-tertiary transition duration-100 ease-linear hover:text-secondary">
